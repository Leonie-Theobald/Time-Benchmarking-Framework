...
#gen DSS keys and pems
for len in 512 1024 2048 3072
do
  #dsa parameters
  openssl genpkey -genparam -algorithm DSS -out dssp${len}.pem -pkeyopt dss_paramgen_bits:${len}
  openssl genpkey -paramfile dssp${len}.pem -out dss${len}_key.pem
  openssl req -new -nodes -subj "/C=DE/ST=NRW/L=Bochum/O=RUB/OU=NDS/CN=localhost" -key dss${len}_key.pem -out dss${len}_key.csr
  #signing with CA keys
  openssl x509 -req -in dss${len}_key.csr -CA attacker_rsa_ca.pem -CAkey attacker_rsa_ca_key.pem -CAcreateserial -out dss${len}_rsa_cert.pem -days 1024 -extfile v3.ext
  openssl x509 -req -in dss${len}_key.csr -CA attacker_dsa_ca.pem -CAkey attacker_dsa_ca_key.pem -CAcreateserial -out dss${len}_dsa_cert.pem -days 1024 -extfile v3.ext
  openssl x509 -req -in dss${len}_key.csr -CA attacker_ecdsa_ca.pem -CAkey attacker_ecdsa_ca_key.pem -CAcreateserial -out dss${len}_ecdsa_cert.pem -days 1024 -extfile v3.ext
  rm dss${len}_key.csr
  rm dssp${len}.pem
done
